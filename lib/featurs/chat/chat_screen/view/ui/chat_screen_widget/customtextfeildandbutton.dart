...
class _CustomTextFeildMessageState extends State<CustomTextFeildMessage> {
  ...
  senddtext() async {
    SharedPref pref;
    pref = SharedPref();
    String uidp = await pref.getfromshared('uid');
    if (widget.collectionname == "chats") {
      ...
      BlocProvider.of<SendMessageGroupCubit>(context).send(
          id: widget.docs,
  ...
  choicesendingtype() {
    showBottomSheet(
        builder: (context) => bottomshet(widget.docs, widget.usermodel,
            widget.collectionname, widget.feildnamel),
        context: context);
  }
